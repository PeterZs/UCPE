import torch
import argparse
from thirdparty.PanoFlowAPI.model.panoflow_raft import PanoRAFT
import sys

sys.path.append('../')
from thirdparty.PanoFlowAPI.utils.padder import InputPadder
from argparse import Namespace
from torch import nn


class PanoRAFTAPI(nn.Module):
    def __init__(self, 
    ...
    def chunk_estimate_flow_cfe(self, imgs, chunk_size=1):
        flow = []
        for prev, curr in zip(
            imgs[:-1].split(chunk_size, dim=0),
            imgs[1:].split(chunk_size, dim=0)
        ):
            flow.append(self._estimate_flow_cfe(prev, curr))
