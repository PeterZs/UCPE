...
def load_i3d_pretrained(device=torch.device('cpu')):
    filepath = "models/FVD/i3d_pretrained_400.pt"
    print(f"loading i3d from {filepath} ...")
    from .pytorch_i3d import InceptionI3d
    i3d = InceptionI3d(400, in_channels=3).eval().to(device)
    i3d.load_state_dict(torch.load(filepath, map_location=device))
    return i3d

def preprocess_single(video, crop_center, resolution, sequence_length=None):
    # video: THWC, {0, ..., 255}
...
def preprocess(videos, crop_center, target_resolution=224):
    # we should tras videos in [0-1] [b c t h w] as th.float 
    # -> videos in {0, ..., 255} [b t h w c] as np.uint8 array
    videos = einops.rearrange(videos, 'b t c h w -> b t h w c')
    videos = torch.stack([preprocess_single(video, crop_center, target_resolution) for video in videos])
    return videos * 2 # [-0.5, 0.5] -> [-1, 1]

def get_fvd_logits(videos, i3d, device, bs=10, crop_center=True):
    videos = preprocess(videos, crop_center)
