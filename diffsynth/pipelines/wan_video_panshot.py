...
import thirdparty.prope.torch as prope
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.units = [
            ...
            WanVideoUnit_UCPECameraControl(),
        ]
        self.post_units = [
...
class WanVideoUnit_ImageEmbedderFused(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_image, latents, height, width, tiled, tile_size, tile_stride, first_frame_latents):
        if (input_image is None and first_frame_latents is None) or not pipe.dit.fuse_vae_embedding_in_latents:
            return {}
...
class WanVideoUnit_UCPECameraControl(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("height", "width", "camera_control_panshot",),
        )

    def process(self, pipe: WanVideoPipeline, height, width, camera_control_panshot):
        if camera_control_panshot is None or getattr(pipe.dit, "camera_condition", "none") == "none":
            return {}
        pose = camera_control_panshot["pose"]
        x_fov = camera_control_panshot["x_fov"]
        xi = camera_control_panshot["xi"]
        method = pipe.dit.camera_condition
        ...
        if method == "recammaster":
            ...
            return {"control_camera_dit_input": {"cam_emb": cam_emb}}
            ...
            plucker_ucm = ucpe.ray_condition_ucm(
                x_fov=x_fov,
                xi=xi,
                pose=pose,
                height=patches_y,
            ...
            return {"control_camera_dit_input": {"cam_emb": plucker_ucm}}
        elif method == "plucker":
            plucker_ucm = ucpe.ray_condition_ucm(
                x_fov=x_fov,
                xi=xi,
            ...
            # === Step 1: Expand dims to match control pipeline (与 equi2pers示例逻辑对齐) ===
            control_camera_video = plucker_ucm.permute(0, 4, 1, 2, 3)  # [1, 6, V, H, W]

            # === Step 2: 构建重复 block（与参考代码逻辑一致） ===
            # 通道 0 重复4次，然后拼上剩余通道
            control_camera_latents = torch.concat(
                [
                    torch.repeat_interleave(control_camera_video[:, :, 0:1], repeats=4, dim=2),
            ...
            # === Step 3: 重排 + reshape 成 ControlNet 接受的 block 结构 ===
            ...
            # === Step 4: Cast to device & dtype of pipeline ===
            ...
            c2w = repeat(c2w, "... -> B T ...", B=pose.shape[0], T=pose.shape[1]).clone()
            c2w[..., :3, :4] = pose
            if "gta" in method or "prope" in method:
                viewmats = prope._invert_SE3(c2w)
            elif "relray" in method:
                attn = pipe.dit.blocks[0].cam_self_attn
                d_cam = ucpe.ucm_unproject_grid_fov(
                    x_fov=x_fov,
                    xi=xi,
                    height=attn.patches_y,
                    width=attn.patches_x,
                    device=pipe.device,
                    dtype=pose.dtype,
                ) # [B, H, W, 3]
                raymats = ucpe.world_to_ray_mats(d_cam, c2w)  # [B, T, H, W, 4, 4]
            ...
            if method == "prope":
                ...
                Ks[..., 1, 2] = height * 0.5
                Ks[..., 2, 2] = 1.0
                control_camera_dit_input["K"] = Ks

            if "absc2w" in method:
                cam_emb = rearrange(pose[:, ::4], "B T ... -> B T (...)")
                control_camera_dit_input["cam_emb"] = cam_emb
            elif "absray" in method:
                cam_emb = viewmats[..., :3, :]
                control_camera_dit_input["cam_emb"] = rearrange(cam_emb, "B N ... -> B N (...)")
